...
void ParticleMgr::startParticle(int type, glRGBA color, std::string t,float scale, float endScale, long lifetime, Vector ps, Vector v)
{
	int pindex=-1;
		for(int i=0;i<MAX_PARTICLES;i++)
...
void ParticleMgr::handle()
{
	...
	for(int i=0;i<MAX_PARTICLES;i++)
	{
		if(particles[i].state==1)
		{
			...
			if(!Camera::get().sphereInFrustum(temp,particles[i].curScale*.03))particles[i].state=0;
			else {
				numParticles++;
...
void ParticleMgr::startExplosion(std::string t, int numParticles, float scale, float endScale, long lifetime, Vector pos, Vector velocity, float explosionSpeed)
{
	...
	for(int i=0;i<part;i++)
	{
		for(int ii=0;ii<part;ii++)
		{
			...
			startParticle(1,(glRGBA){rand()%10/10.0f,0.2f,rand()%10/10.0f,1.0f},t,scale, endScale, lifetime, pos, velocity+v);
		}
	}
}
