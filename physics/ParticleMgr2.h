...
class ParticleMgr2 : public Singleton<ParticleMgr2> {
  class ParticleGroup {
   ...
   public:
    ...
    ~ParticleGroup();
    std::string getID();
    void startParticle(int type, glRGBA color, float scale, float endScale,
                       long lifetime, Vector ps, Vector v);
    void render();
    void handle();
  };
 ...
 public:
  ...
  void startParticle(std::string groupID, int type, glRGBA color, float scale,
