...
void ObjectMgr::handle()
{
	...
	while(!br){
		...
		if(!objectList->currentElement()) break;
		...
		if(o->done && !delayDestruction)el=objectList->getCurrent();
...
void ObjectMgr::renderPlayer(bool showModel)
{
	...
	if(player!=NULL)
	{
		...
		if(showModel)
		{
			...
			Vector offset = (Camera::get().up*1.0f+Camera::get().look*3.0f)*-1;
...
Object *ObjectMgr::detectCollision(Object *obj)
{
	...
	while(1){
		...
		if(!objectList->nextElement()) break;
	}
	objectList->setCurrent(el);
	return NULL;
}
