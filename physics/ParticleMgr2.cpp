...
void ParticleMgr2::addParticleGroup(std::string id, std::string texture,
                                    int numParticles) {
  groups->addElement(new ParticleGroup(id, texture, numParticles));
}

void ParticleMgr2::startParticle(std::string groupID, int type, glRGBA color,
                                 float scale, float endScale, long lifetime,
                                 Vector ps, Vector v) {
  ParticleGroup *g = groups->find(groupID);
  if (g)
    g->startParticle(type, color, scale, endScale, lifetime, ps, v);
}
...
ParticleMgr2::ParticleGroup::ParticleGroup(std::string id, std::string texture,
                                           int numParticles) {
  this->id = id;
...
void ParticleMgr2::ParticleGroup::startParticle(int type, glRGBA color,
                                                float scale, float endScale,
                                                long lifetime, Vector ps,
                                                Vector v) {
  int pindex = -1;
  ...
  for (int i = 0; i < numParticles; i++) {
    if (particles[i].state == 1) {
      if (!Camera::get().sphereInFrustum(
          Vector(particles[i].pos.x, particles[i].pos.y, particles[i].pos.z),
          particles[i].curScale * .03))
        particles[i].state = 0;
      else {
        numParticles++;
