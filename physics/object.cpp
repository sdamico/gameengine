...
Object::Object(std::string id, std::string model, Vector pos, Vector velocity,
               double mass, float scale) {
  objectType = "object";
  build(id, model, pos, velocity, mass, scale);
}

void Object::build(std::string id, std::string model, Vector pos,
  ...
  if (mod)
    this->size = mod->getSphereRadius();
  else
    this->size = 1.0f;

  //Estimate I as a solid cylinder:
  this->momentOfInertia = 0.5f * this->mass * (this->size * this->scale)
...
Vector Object::detectCollision(Vector x1, Vector x2) {
  if ((x1 - pos).magnitude()) {
    double distance = ((x2 - x1) % (x1 - pos)).magnitude()
        / (x2 - x1).magnitude();
