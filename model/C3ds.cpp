...
C3ds::C3ds(std::string id, std::string file, std::string shaderProgram,
           float scale) {
  fileSize = 0;
  data = NULL;
  vertex = NULL;
...
void C3ds::calcNormals() {
  ...
  for (int i = 0; i < numFaces; i++) {
    fnormal[i].calcNormal(vertex[face[i].p1], vertex[face[i].p2],
                          vertex[face[i].p3]);
 ...
 * Note - do not use - needs to be fixed
 */
bool C3ds::intersect(C3ds *other, float scale, float otherScale,
                     FVector separation, Quaternion q1, Quaternion q2) {
  if (!other)
    return false;
  ...
  for (int i = 0; i < other->numFaces; i++) {
    ...
    otherTemp[0] =
        (FVector) {rotOther[0]*other->vertex[other->face[i].p1]*otherScale,rotOther[1]*other->vertex[other->face[i].p1]*otherScale,rotOther[2]*other->vertex[other->face[i].p1]*otherScale};
    otherTemp[1] =
        (FVector) {rotOther[0]*other->vertex[other->face[i].p2]*otherScale,rotOther[1]*other->vertex[other->face[i].p2]*otherScale,rotOther[2]*other->vertex[other->face[i].p2]*otherScale};
    otherTemp[2] =
        (FVector) {rotOther[0]*other->vertex[other->face[i].p3]*otherScale,rotOther[1]*other->vertex[other->face[i].p3]*otherScale,rotOther[2]*other->vertex[other->face[i].p3]*otherScale};
    if ((otherTemp[0] + separation).magnitude() < radius
        || (otherTemp[1] + separation).magnitude() < radius
        || (otherTemp[2] + separation).magnitude() < radius) {
      u = otherTemp[1] - otherTemp[0];
      v = otherTemp[2] - otherTemp[0];
      w = otherTemp[2] - otherTemp[1];
      otherNormalTemp =
          (FVector) {rotOther[0]*other->fnormal[i],rotOther[1]*other->fnormal[i],rotOther[2]*other->fnormal[i]};
      for (int j = 0; j < numFaces; j++) {
        selfTemp[0] =
            (FVector) {rot[0]*vertex[face[j].p1]*scale,rot[1]*vertex[face[j].p1]*scale,rot[2]*vertex[face[j].p1]*scale};
        selfTemp[1] =
            (FVector) {rot[0]*vertex[face[j].p2]*scale,rot[1]*vertex[face[j].p2]*scale,rot[2]*vertex[face[j].p2]*scale};
        selfTemp[2] =
            (FVector) {rot[0]*vertex[face[j].p3]*scale,rot[1]*vertex[face[j].p3]*scale,rot[2]*vertex[face[j].p3]*scale};
...
FVector C3ds::intersect(FVector x1, FVector x2, float scale, Quaternion q1) {
  ...
  for (int i = 0; i < numVerts; i++) {
    ...
    float distance = ((x2 - x1) % (x1 - selfTemp)).magnitude()
        / (x2 - x1).magnitude();
