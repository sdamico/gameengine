...
WeaponDisplay::WeaponDisplay(Object *parent, std::string texture,
...
void WeaponDisplay::render() {
  if (parent && parent->getObjectType() == "player") {
    ...
    glMaterialfv(GL_FRONT, GL_EMISSION,
                 (const float[4]) {1.0f,1.0f,1.0f,1.25f});
    glEnable(GL_BLEND);
    ...
    glTranslatef(Camera::get().up.x * -0.30f, Camera::get().up.y * -0.30f,
                 Camera::get().up.z * -0.30f);
    ...
    if (curLauncher) {
      ...
      FontLib::get().glPrint("Remaining Ammunition: %d/%d",
                             curLauncher->getAmmo(), curLauncher->getMaxAmmo());
    } else {
      glRasterPos3f(-0.95f, -0.36f, -0.5f);
