...
SpaceObject::SpaceObject(std::string id, std::string factionID,
                         std::string model, Vector pos, Vector velocity,
...
void SpaceObject::track(Vector p) {
  ...
  //Vector from your position to object being tracked
  Vector delta = p - pos;
  float seconds = ((-1 * velocity.magnitude()
      + sqrt(
          velocity.magnitude() * velocity.magnitude()
              + 2 * topAccel * delta.magnitude())) / (topAccel));
  float seconds2 = ((-1 * velocity.magnitude()
      - sqrt(
          velocity.magnitude() * velocity.magnitude()
              + 2 * topAccel * delta.magnitude())) / (topAccel));
