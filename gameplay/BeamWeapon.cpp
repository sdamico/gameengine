...
void BeamWeapon::detectCollision() {
  if (source && beamWidth) {
    Object *o = ObjectMgr::get().detectCollision(
        source->getPos(), source->getPos() + source->getLook() * BEAM_LENGTH);
    if (o) {
...
void BeamWeapon::render() {
  if (source && beamWidth && !overHeat && on) {
    ...
    glTranslatef(position.z - 1.0f,
                 position.y + ((double) (rand() % 1000)) / 50000.0f,
                 position.x);
