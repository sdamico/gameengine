...
Missile::Missile(std::string id, std::string model, Vector pos, Vector velocity,
                 float launchSpeed, Quaternion q, std::string source,
                 std::string target, double mass, float scale)
  ...
  if (parentObj)
    factionID = parentObj->getFactionID();
  else
    factionID = "";
...
void Missile::handleCollision(Object *other) {
  if (other == NULL || other->getID() != source) {
    ...
    ParticleMgr::get().startExplosion("engine", 225, 0.1f, 4000.0f, 700, pos,
                                      velocity, 300);
  }
}
...
void Missile::action() {
  ...
  if (SDL_GetTicks() - startTime < homeTime) {
    track(target);
    ParticleMgr::get().startParticle(
        1,
        (glRGBA) {0.2f,0.2f,1.0f,1.0f},
        "engine",
        size * scale * 3.0f,
        size * scale * 6.0f,
        100,
        pos - look * size * scale * .5,
        velocity - look * 15
            + Vector((((float) (rand() % 100)) - 50.0f) / 10000.0f,
                     (((float) (rand() % 100)) - 50.0f) / 10000.0f,
                     (((float) (rand() % 100)) - 50.0f) / 1000.0f));
  }
}
