...
void TargetingSystem::targetNearest()
{
	...
	if(list)
	{
		...
		while(1){
			...
			{
				double newDist = (list->currentElement()->getPos()-parent->getPos()).magnitude();
...
void TargetingSystem::targetNearestEnemy()
{
	...
	if(list)
	{
		...
		while(1){
			//if(obj!=objectList->currentElement() && source!=objectList->currentElement()){
			if(parent->getID()!=list->currentElement()->getID() && list->currentElement()->getObjectType()!="missile" && list->currentElement()->getObjectType()!="object" && list->currentElement()->getObjectType()!="gravitywell" && (list->currentElement()->getFactionID()=="" || parent->getFactionID()!=list->currentElement()->getFactionID()))
			{
				double newDist = (list->currentElement()->getPos()-parent->getPos()).magnitude();
				if(newDist<dist || !dist)
				{
...
void TargetingSystem::targetNearestAlly()
{
	...
	if(list)
	{
		...
		while(1){
			//if(obj!=objectList->currentElement() && source!=objectList->currentElement()){
			if(parent->getID()!=list->currentElement()->getID() && list->currentElement()->getObjectType()!="missile" && list->currentElement()->getObjectType()!="object" && list->currentElement()->getObjectType()!="gravitywell" && parent->getFactionID()==list->currentElement()->getFactionID())
			{
				double newDist = (list->currentElement()->getPos()-parent->getPos()).magnitude();
				if(newDist<dist || !dist)
				{
...
void TargetingSystem::targetNearestMissile()
{
	...
	if(list)
	{
		...
		while(1){
			//if(obj!=objectList->currentElement() && source!=objectList->currentElement()){
			if(parent->getID()!=list->currentElement()->getID() && list->currentElement()->getObjectType()=="missile")
			{
				double newDist = (list->currentElement()->getPos()-parent->getPos()).magnitude();
				if(newDist<dist || !dist)
				{
...
{
	if(o && Camera::get().sphereInFrustum(o->getPos(),o->getSphereRadius()) && m)
	{
		glPushMatrix();
		glLoadIdentity();
		...
		if(o->getSphereRadius()*0.5f*sizeDecrease>0.01f)
			glScalef(o->getSphereRadius()*0.5f*sizeDecrease,o->getSphereRadius()*0.5f*sizeDecrease,o->getSphereRadius()*0.5f*sizeDecrease);
		else
...
void TargetingSystem::drawTargetingBoxes()
{
	...
	if(list)
	{
		...
		while(1)
		{
			...
			else if(!(obj->getObjectType()=="missile" || obj->getObjectType()=="gravitywell" || obj->getObjectType()=="object"))
			{
...
void TargetingSystem::render()
{
	...
	glTranslatef(Camera::get().up.x*-0.55f,Camera::get().up.y*-0.55f,Camera::get().up.z*-0.55f);
	glTranslatef(Camera::get().right.x*0.60f,Camera::get().right.y*0.60f,Camera::get().right.z*0.60f);
	...
	if(t)
	{
		...
		FontLib::get().glPrint("<%0.2f, %0.2f, %0.2f>", relVelocity.x,relVelocity.y,relVelocity.z);
		...
		FontLib::get().glPrint("<%0.2f, %0.2f, %0.2f>", toTarget.x,toTarget.y,toTarget.z);
		glRasterPos3f(0.4f,-0.98f,-0.5f);
		if(t->getObjectType()=="ship" || t->getObjectType()=="player")
		{
			FontLib::get().glPrint("Current Target: %s", ((Ship*)t)->getTargetID().c_str());
		} else if(t->getObjectType()=="missile")
		{
			FontLib::get().glPrint("Current Target: %s", ((Missile*)t)->getTargetID().c_str());
		}
	}
	else
	{
