...
WeaponLauncher::WeaponLauncher(std::string id, int maxAmmo, int curAmmo, unsigned long fireRate, SpaceObject *parent, Vector position, Quaternion direction)
{
	this->id=id;
...
bool WeaponLauncher::launch()
{
	if(curAmmo>0 && parent && (SDL_GetTicks()-lastLaunch)>fireRate)
	{
		...
		ObjectMgr::get().add(new Missile(projectileName.str(),
										 "missile",
										 parent->getPos()+(parent->getRight()*position.x+parent->getUp()*position.y+parent->getLook()*position.z),
										 parent->getVelocity(),
										 50.0f,
										 direction*parent->getQuaternion(),
										 parent->getID(),
										 parent->getTargetID(),
										 100.0f,
										 0.3f));
