...
Ship::Ship(std::string id, std::string factionID, std::string model, Vector pos, Vector velocity, double mass, float scale)
	: SpaceObject(id,factionID,model,pos,velocity,mass,scale)
{
	...
	weaponLauncher = new WeaponLauncher("Missile Launcher", 20, 20, 1000, this, Vector(0.1f*size*scale,0.0f,0.0f), Quaternion(0,0,0,1));
...
void Ship::handleCollision(Object *other)
{
	...
	else if(other->getSource()!=id)
	{
		...
		ParticleMgr::get().startExplosion("engine",36,0.1f,500.0f,700,pos,velocity, 10);
	}
...
void Ship::action()
{
	...
	if(p)
	{
		...
		if(delta<100000.0f)
		{
			if(((p->getVelocity()-velocity).magnitude()<delta/6.0f) || delta>5000.0f)
				track(p->getID());
			else if((p->getVelocity()-velocity).magnitude()<50.0f)
				matchVelocity(p->getID());
			else pointTowards(p->getID());
	...
	else
	{
		...
		{
			if(velocity.magnitude()>50.0f)stop();
			else track(Vector());
		}
		targeting->targetNearestEnemy();
	}
...
{
	if(targeting) return targeting->getTargetID();
	return "";
}
