...
Player::Player(std::string pid, std::string factionID, std::string model,
               Vector pos, Vector velocity, Quaternion q, double mass,
               float scale)
  ...
  weaponLauncher = new WeaponLauncher(
      "Missile Launcher", 20, 20, 1000, this,
      Vector(0.15f * size * scale, -0.05f * size * scale, 0.0f),
      Quaternion(0, 0, 0, 1));
...
void Player::action() {
  ...
  Camera::get().setPos(
      pos + Camera::get().up * -1.0f + Camera::get().look * -3.0f);
