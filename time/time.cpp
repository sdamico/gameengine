...
void GlobalTime::calcDelta() {
  ...
  frameSec = ((((long double) end->tv_usec)) - ((long double) start->tv_usec))
      * 0.000001f;
  if (frameSec < 0)
    frameSec = ((long double) (1000000 - start->tv_usec)
        + (long double) end->tv_usec) * 0.000001f;
...
GlobalTime::~GlobalTime() {
  ...
  delete end;
  //Output average FPS
  std::cout << "[Ending Global Timer] ["
            << 1000 * (((float) frames) / (float) (SDL_GetTicks() - startTime))
            << " FPS]" << std::endl;
}
