...
bool Texture::load(std::string nid, std::string file) {
  ...
  if (image->getImageFormat() == IMAGE_RGB) {
    ...
    gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB, image->getWidth(),
                      image->getHeight(), GL_RGB, GL_UNSIGNED_BYTE,
                      image->getImage());
  }
  ...
  if (image->getImageFormat() == IMAGE_LUMINANCE) {
    ...
    gluBuild2DMipmaps(GL_TEXTURE_2D, GL_LUMINANCE, image->getWidth(),
                      image->getHeight(), GL_LUMINANCE, GL_UNSIGNED_BYTE,
                      image->getImage());
  }
