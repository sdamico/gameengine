...
void Camera::calcFrustum()
{
    ...
    size = sqrt(ViewFrustum.planes[0].A*ViewFrustum.planes[0].A + ViewFrustum.planes[0].B*ViewFrustum.planes[0].B + ViewFrustum.planes[0].C*ViewFrustum.planes[0].C);
...
bool Camera::pointInFrustum(Vector p)
{
	...
	for(int i=0;i<6;i++)
	{
		if((ViewFrustum.planes[i].A*p.x+ViewFrustum.planes[i].B*p.y+ViewFrustum.planes[i].C*p.z+ViewFrustum.planes[i].D)<0)
			return false;
	}
	return true;
}
...
bool Camera::sphereInFrustum(Vector p, float radius)
{
	...
	for(int i=0;i<6;i++)
	{
		dist=ViewFrustum.planes[i].A*p.x+ViewFrustum.planes[i].B*p.y+ViewFrustum.planes[i].C*p.z+ViewFrustum.planes[i].D;
		if(dist<=-radius){
